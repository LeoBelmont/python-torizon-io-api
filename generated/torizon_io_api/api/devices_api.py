...
class DevicesApi:
    ...
    def get_devices_packages(
        ...
        """Get information about the installed packages for many devices
         ...
         Returns a list of devices and the packages those devices have installed. A list of devices can be specified as a query parameter; if no devices are specified will return information for all devices in the repository.  Packages are reported according to the following process:  1. The image repository is searched using the package hash provided by the device. 2. The search results are evaluated according to these rules:     - **Single match**      If exactly one package is found matching the hash, this package will be reported.     - **Multiple matches**      - If one of the matching packages also matches the `packageId` reported by the device, that package will be reported.      - If none of the matching packages correspond to the device’s `packageId`, the package with the first `packageId` in alphabetical order will be reported.     - **No matches**      If no packages are found matching the hash, it will be reported that the package is not present in the repository. 
    ...
    def get_devices_packages_deviceuuid(
        ...
        Returns a list of the packages installed on the device.  Package reporting logic implemented as defined in: [Devices/getDevicesPackages](#/Devices/getDevicesPackages) \"
    ...
    def post_devices_packages(
        ...
        """Get information about the installed packages for many devices
         ...
         Returns a list of devices and the packages those devices have installed. A list of devices can be specified using the request body; if no devices are specified will return information for all devices in the repository.  The number of provided deviceUuids in the request body is limited to 100  Package reporting logic implemented as defined in: [Devices/getDevicesPackages](#/Devices/getDevicesPackages) 
